...
/**
 * ViewModel responsável pela gestão do estado e lógica de negócio do ecrã Gestor.
 *
 * Implementa o padrão MVI (Model-View-Intent) onde:
 * - **Model**: Representado por [GestorState]
 * - **View**: O Composable [GestorScreen]
 * - **Intent**: As ações definidas em [GestorAction]
 *
...
class GestorViewModel : ViewModel() {
     ...
     */
    private val _state = MutableStateFlow(GestorState())
     ...
     *
     * @param action A ação a ser processada, definida em [GestorAction].
     */
    fun onAction(action: GestorAction) {
        when (action) {
