...
/**
 * ViewModel responsável pela gestão do estado e lógica de negócio do ecrã SSO.
 *
 * Implementa o padrão MVI (Model-View-Intent) onde:
 * - **Model**: Representado por [SSOState]
 * - **View**: O Composable [SSOScreen]
 * - **Intent**: As ações definidas em [SSOAction]
 *
...
class SSOViewModel : ViewModel() {
     ...
     */
    private val _state = MutableStateFlow(SSOState())
     ...
     *
     * @param action A ação a ser processada, definida em [SSOAction].
     */
    fun onAction(action: SSOAction) {
        when (action) {
